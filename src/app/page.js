...
export default function LoveGrid() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (process.env.NODE_ENV === "production") {
        if (current >= birthdayStart && current < birthdayEnd) {
          setShowBirthday(true);
        } else if (current >= birthdayEnd) {
          setShowMain(true);
        }
      }
    }, 1000);
  ...
  if (process.env.NODE_ENV === "production" && showBirthday) {
    return (
  ...
  if (process.env.NODE_ENV === "production" && showMain) {
    return (
